...
pacman::p_load(rvest, tidyverse, stringr, httr, parallel, foreach, doParallel)

# flow control for file existence:
...
total_list <- list()
total_list <- 
  foreach(jj = 1:length(yet_to_process)
  , .packages = c('rvest', 'tidyverse', 'stringr', 'httr')) %dopar% {
...
run_end <- Sys.time()
tot_time <- run_end - run_start # 1.5 mins on 10 cores
tot_time
